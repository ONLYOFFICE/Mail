...
public class MailQueueItemSettings
{
    ...
    private static Dictionary<string, int> GetDefaultFolders()
    {
        ...
        try
        {
            if (!string.IsNullOrEmpty(System.Configuration.ConfigurationManager.AppSettings["mail.folders-mapping"]))
            {
                list = System.Configuration.ConfigurationManager.AppSettings["mail.folders-mapping"]
                    .Split('|')
                    .Select(s => s.Split(':'))
